...
#elif PLATFORM_ANDROID
        size_t length = std::strlen(DATADIR);
        char* newDataDir = new char[length+1];
        std::strcpy(newDataDir, DATADIR);
        return newDataDir;
#else
...
const std::string& getSystemDocDir()
{
	...
	if (!result) result = []() -> std::string {
		...
		return getAbsolutePath("openmsx"); // TODO: currently no docs are installed on Android
#else
