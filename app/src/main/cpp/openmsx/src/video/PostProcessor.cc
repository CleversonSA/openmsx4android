...
void PostProcessor::paint(OutputSurface& /*output*/)
{
...
#if defined(__ANDROID__)
    const int guiTop = screen.getGuiTopReservedPixels();

    if ((screen.getViewOffset() != ivec2()) || // any part of the screen not covered by the viewport?
        (guiTop > 0) || // leave room for the UI (e.g. ImGui menu bar)
    ...
    gl::checkGLError("PostProcessor::paint0");
    ...
    //__android_log_print(ANDROID_LOG_INFO,"MVP0-FB","FBO status = 0x%04x", st);
	...
	if (renderSettings.getFullStretch()) {
		...
		auto [x0, y0] = screen.getViewOffset();
		auto [w, h] = screen.getViewSize();
        int x = x0;
        int y = y0;
#if defined(__ANDROID__)
        if (guiTop > 0) {
            auto [pw, ph] = screen.getPhysicalSize();
            int maxTop = ph - guiTop;
            if (y + h > maxTop) y = maxTop - h;
        }
        // OpenGL XY are from bottom left not top left
        int topY = 50;
...
void PostProcessor::drawGlow(int glow)
{
    ...
    __android_log_print(ANDROID_LOG_INFO, "MVP0-drawGlow",
                        "SDL current ctx=%p", SDL_GL_GetCurrentContext());
...
void PostProcessor::drawNoise() const
{
    ...
    __android_log_print(ANDROID_LOG_INFO, "MVP0-drawNoise",
                        "SDL current ctx=%p", SDL_GL_GetCurrentContext());
